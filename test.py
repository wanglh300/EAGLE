...
def evaluate_gen(model, args):
    ...
    save_dir = os.path.dirname(args.resume_checkpoint)
    np.save(os.path.join(save_dir, "model_out_smp_airplane.npy"), sample_pcs.cpu().detach().numpy())
    np.save(os.path.join(save_dir, "model_out_ref_airplane.npy"), ref_pcs.cpu().detach().numpy())
