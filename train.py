...
import time
import scipy.misc
from models.networks import EAGLE
    ...
    if args.distributed:  # Multiple processes, single GPU per process
        if args.gpu is not None:
            def _transform_(m):
                ...
                #     m, device_ids=[args.gpu], output_device=args.gpu,find_unused_parameters=True)
                if any(p.requires_grad for p in m.parameters()):
                    return nn.parallel.DistributedDataParallel(
                        m, device_ids=[args.gpu], output_device=args.gpu,find_unused_parameters=True)
                else:
                    print("No parameters require gradients. Skipping DistributedDataParallel.")
                    return m
        ...
        for bidx, data in enumerate(train_loader):
            ...
            if step % args.log_freq == 0:
                ...
                print("[Rank %d] Epoch %d Batch [%2d/%2d] Time [%3.2fs] Entropy %2.5f LatentNats %2.5f PointNats %2.5f"
                      % (args.rank, epoch, bidx, len(train_loader)-1, duration, entropy_avg_meter.avg,
