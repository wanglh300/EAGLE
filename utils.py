...
def validate_classification(loaders, model, args,dataset_name):
    ...
    res = {'acc': acc}
    print(f"Acc for {dataset_name}: {acc}")  # 修改输出，显示数据集名称
    return res
...
def validate(test_loader, model, epoch, writer, save_dir, args, clf_loaders=None):
    ...
    if args.eval_classification and clf_loaders is not None:
        for clf_expr, loaders in clf_loaders.items():
            dataset_name = "ModelNet40" if clf_expr == 'MN40' else "ModelNet10"  # 判断数据集名称
            with torch.no_grad():
                clf_val_res = validate_classification(loaders, model, args,dataset_name)
