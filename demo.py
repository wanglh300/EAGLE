...
def main(args):
    ...
    np.save(os.path.join("demo", "model_out_smp_airplane.npy"), sample_pcs)

    # Save point clouds to PLY files
    for i in range(int(sample_pcs.shape[0])):
        print("Saving point cloud: %03d/%03d" % (i, sample_pcs.shape[0]))
        pts = sample_pcs[i].reshape(-1, 3)
        filename = os.path.join("demo", f"sample_{i:03d}.ply")
        save_point_cloud(pts, filename)
